...
describe('Model Switching Integration', () => {
  ...
  test('embedding service tracks active model ID', async () => {
    ...
    const modelRow = await db.execute('SELECT * FROM embedding_models WHERE id = ?', [modelId]);
  ...
  test('embeddings are stored with model reference', async () => {
    ...
    const vectorRow = await db.execute('SELECT * FROM memory_vectors WHERE memory_id = ?', [memory.id]);
