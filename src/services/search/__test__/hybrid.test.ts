...
describe('Hybrid Search', () => {
  ...
  test('filters by minimum salience', async () => {
    const lowSalienceMem = await store.create({ content: 'Low salience memory for testing minimum filter' }, 'proj1');
    ...
    expect(results.every(r => r.memory.salience >= 0.5)).toBe(true);
  });

  test('includes related memory count', async () => {
    const mem1 = await store.create({ content: 'First memory for relationship test' }, 'proj1');
    const mem2 = await store.create({ content: 'Second memory for relationship test' }, 'proj1');
  ...
  test('excludes superseded memories by default', async () => {
    const oldMem = await store.create({ content: 'Old API endpoint is at version one' }, 'proj1');
    const newMem = await store.create({ content: 'New API endpoint is at version two' }, 'proj1');
  ...
  test('includes superseded when requested', async () => {
    const oldMem = await store.create({ content: 'Outdated information about configuration' }, 'proj1');
    const newMem = await store.create({ content: 'Updated information about configuration' }, 'proj1');
    await supersede(oldMem.id, newMem.id);
  ...
  test('boosts salience on retrieval', async () => {
    const mem = await store.create({ content: 'Memory to be boosted by search' }, 'proj1');
...
describe('Timeline', () => {
  ...
  test('returns anchor memory', async () => {
    const anchor = await store.create({ content: 'Anchor memory for timeline' }, 'proj1');
  ...
  test('throws for non-existent anchor', async () => {
    expect(search.timeline('non-existent-id')).rejects.toThrow('Anchor memory not found');
  });
});
...
describe('Session Context', () => {
  ...
  beforeEach(async () => {
    ...
    await db.execute(`INSERT INTO sessions (id, project_id, started_at, summary) VALUES (?, ?, ?, ?)`, [
      'sess1',
      'proj1',
      now,
  ...
  test('returns session context for memory', async () => {
    const memory = await store.create({ content: 'Memory with session context' }, 'proj1', 'sess1');
  ...
  test('returns null for memory without session', async () => {
    const memory = await store.create({ content: 'Memory without session' }, 'proj1');
  ...
  test('includes session summary', async () => {
    const memory = await store.create({ content: 'Memory for summary test' }, 'proj1', 'sess1');
...
describe('Ranking', () => {
  ...
  test('higher salience memories rank higher', async () => {
    const lowSalience = await store.create({ content: 'Low salience ranking test memory' }, 'proj1');
    const highSalience = await store.create({ content: 'High salience ranking test memory' }, 'proj1');
...
describe('Degraded Mode (no embedding service)', () => {
  ...
  test('falls back to keyword search when no embedding service', async () => {
    await store.create({ content: 'React TypeScript frontend application' }, 'proj1', 'sess1');
  ...
  test('handles hybrid mode request gracefully', async () => {
    await store.create({ content: 'Testing graceful degradation with hybrid mode' }, 'proj1', 'sess1');
  ...
  test('handles semantic mode request gracefully', async () => {
    await store.create({ content: 'Semantic search fallback test' }, 'proj1', 'sess1');
  ...
  test('keyword mode works normally', async () => {
    await store.create({ content: 'Keyword only mode test memory' }, 'proj1', 'sess1');
  ...
  test('timeline works without embedding service', async () => {
    const anchor = await store.create({ content: 'Anchor for degraded timeline' }, 'proj1', 'sess1');
  ...
  test('getSessionContext works without embedding service', async () => {
    const memory = await store.create({ content: 'Memory for session context test' }, 'proj1', 'sess1');
