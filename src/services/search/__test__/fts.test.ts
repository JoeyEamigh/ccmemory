...
describe("FTS5 Search", () => {
  ...
  beforeEach(async () => {
    ...
    await store.create(
      {
        content:
          "The authentication module handles user login and JWT tokens for secure access",
      },
      "proj1"
    );
    await store.create(
      {
        content:
          "Database migrations are run with the migrate command to update schema",
      },
      "proj1"
    );
  });
  ...
  test("filters by project", async () => {
    await store.create(
      { content: "authentication in another project for different users" },
      "proj2"
    );
  ...
  test("finds multiple matching memories", async () => {
    await store.create(
      { content: "User management with authentication tokens for API access" },
      "proj1"
    );
  ...
  test("excludes deleted memories", async () => {
    const mem = await store.create(
      { content: "authentication system for deleted content" },
      "proj1"
    );
