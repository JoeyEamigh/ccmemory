...
describe("MemoryStore", () => {
  ...
  describe("create", () => {
    ...
    test("respects explicit sector", async () => {
      const memory = await store.create(
        { content: "Some content", sector: "episodic" },
        "proj1"
      );
    ...
    test("extracts concepts from content", async () => {
      const memory = await store.create(
        { content: "Use `getUserById` function from src/users/api.ts" },
        "proj1"
      );
...
describe("extractConcepts", () => {
  test("extracts backtick code references", async () => {
    ...
    await db.execute(
      `INSERT INTO projects (id, path, name, created_at, updated_at) VALUES (?, ?, ?, ?, ?)`,
      ["proj1", "/test", "Test", now, now]
    );

    const memory = await store.create(
      { content: "Call `handleAuth` and `processRequest`" },
      "proj1"
    );
  ...
  test("extracts file paths", async () => {
    ...
    const memory = await store.create(
      { content: "Located in /src/auth/handler.ts" },
      "proj1"
    );
