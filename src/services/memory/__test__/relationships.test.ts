...
describe('Memory Relationships', () => {
  ...
  async function createTestMemory(id: string, content: string, projectId = 'proj1'): Promise<void> {
    const now = Date.now();
    await db.execute(
  ...
  test('createRelationship with custom confidence', async () => {
    ...
    const rel = await createRelationship('mem1', 'mem2', 'RELATED_TO', 'llm', 0.85);
  ...
  test('supersede marks old memory with valid_until', async () => {
    ...
    const result = await db.execute('SELECT valid_until FROM memories WHERE id = ?', ['old']);
  ...
  test('invalidateRelationship sets valid_until', async () => {
    ...
    const result = await db.execute('SELECT valid_until FROM memory_relationships WHERE id = ?', [rel.id]);
    expect(result.rows[0]?.['valid_until']).toBeDefined();
  });
});
