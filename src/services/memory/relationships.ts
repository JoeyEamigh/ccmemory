...
export async function getRelationships(memoryId: string): Promise<MemoryRelationship[]> {
  const db = await getDatabase();
...
export async function getRelatedMemories(memoryId: string, relationshipType?: RelationshipType): Promise<Memory[]> {
  const db = await getDatabase();
...
export async function supersede(oldMemoryId: string, newMemoryId: string): Promise<void> {
  const db = await getDatabase();
  const now = Date.now();
...
export async function getSupersedingMemory(memoryId: string): Promise<Memory | null> {
  const db = await getDatabase();
...
export async function invalidateRelationship(relationshipId: string): Promise<void> {
  ...
  await db.execute(`UPDATE memory_relationships SET valid_until = ? WHERE id = ?`, [now, relationshipId]);
}
