...
import { checkIntegrity, checkpointWAL, createDatabase } from '../database.js';
...
describe('Database', () => {
  ...
  test('executes parameterized queries', async () => {
    const db = await createDatabase(':memory:');
    await db.execute('INSERT INTO projects (id, path) VALUES (?, ?)', ['p1', '/test/path']);
    const result = await db.execute('SELECT * FROM projects WHERE id = ?', ['p1']);
  ...
  test('batch operations rollback on error', async () => {
    const db = await createDatabase(':memory:');
    await db.execute('INSERT INTO projects (id, path) VALUES (?, ?)', ['p1', '/path1']);
...
describe('Database Recovery', () => {
  ...
  test('checkIntegrity checks all tables', async () => {
    ...
    await db.execute(`INSERT INTO projects (id, path, name) VALUES (?, ?, ?)`, ['p1', '/test', 'Test Project']);
