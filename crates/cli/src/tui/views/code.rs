...
use ccengram::ipc::code::{CodeItem, CodeStatsResult};
use ratatui::{
  buffer::Buffer,
...
impl CodeState {
  ...
  pub fn files(&self) -> Vec<(&str, Vec<&CodeItem>)> {
...
impl CodeView<'_> {
  fn render_file_tree(&self, area: Rect, buf: &mut Buffer) {
    ...
    for (file_path, chunks) in files.iter() {
      ...
      let language = chunks.first().and_then(|c| c.language.as_deref()).unwrap_or("unknown");
