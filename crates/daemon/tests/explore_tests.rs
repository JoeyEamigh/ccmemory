...
async fn test_explore_expand_top_zero() {
  ...
  for r in results {
    assert!(
      r["context"].is_null(),
      "Result should not have context when expand_top=0"
    );
  }
}
...
"#,
  ...
  for r in results {
    ...
    assert!(score <= prev_score, "Results should be sorted by score descending");
    prev_score = score;
  }
}
...
async fn test_context_prefix_match() {
  ...
  if !results.is_empty() {
    ...
    let context_response = router.handle(context_request).await;
    assert!(
      context_response.result.is_some(),
      "8-char prefix should resolve for code chunks"
    );
  }
}
