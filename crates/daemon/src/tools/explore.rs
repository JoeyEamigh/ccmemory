...
impl ToolHandler {
    ...
    pub async fn explore(&self, request: Request) -> Response {
        ...
        if use_text {
            Response::success(request.id, serde_json::Value::String(format_explore_response(&response)))
        } else {
    ...
    pub async fn context(&self, request: Request) -> Response {
        ...
        if ids.len() > max_batch {
            return Response::error(
                request.id,
                -32602,
                &format!("Too many IDs (max {}). Use multiple calls for larger batches.", max_batch),
            );
        }
        ...
        let use_text = args.format.as_deref() == Some("text");
        if use_text {
            Response::success(request.id, serde_json::Value::String(format_context_response(&response)))
        } else {
            Response::success(request.id, serde_json::to_value(response).unwrap_or_default())
        }
    }
    ...
    async fn compute_code_hints(&self, db: &db::ProjectDb, chunk: &CodeChunk) -> ExploreHints {
        ...
        let siblings = db
            .list_code_chunks(Some(&format!("file_path = '{}'", chunk.file_path.replace('\'', "''"))), None)
            .await
    ...
    async fn count_callers(&self, db: &db::ProjectDb, chunk: &CodeChunk) -> usize {
        ...
        for symbol in &chunk.symbols {
            ...
            if let Ok(chunks) = db.list_code_chunks(Some(&format!("calls LIKE '{}'", pattern)), Some(100)).await {
                count += chunks.len();
            }
        }
        count
    ...
    async fn get_callees(&self, db: &db::ProjectDb, chunk: &CodeChunk, limit: usize) -> Vec<CallInfo> {
        ...
        for call in &chunk.calls {
            ...
            if let Ok(chunks) = db
                .list_code_chunks(
                    Some(&format!("symbols LIKE '%{}%'", call.replace('\'', "''"))),
                    Some(3),
                )
                .await
            {
    ...
    async fn fetch_context(&self, db: &db::ProjectDb, id: &str, depth: usize) -> Result<ContextResult, String> {
        ...
        match db.get_code_chunk_by_id_or_prefix(id).await {
            ...
            Err(db::DbError::AmbiguousPrefix { prefix, count }) => {
                return Err(format!("Ambiguous prefix '{}' matches {} items. Use more characters.", prefix, count));
            }
            _ => {}
        }
        ...
        {
            return Ok(ContextResult::Memory(self.build_memory_context(db, memory, depth).await));
        }
    ...
    async fn get_related_memories_for_memory(&self, db: &db::ProjectDb, memory: &Memory, limit: usize) -> Vec<RelatedMemoryInfo> {
