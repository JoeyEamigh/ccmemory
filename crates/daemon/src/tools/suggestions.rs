...
pub fn generate_suggestions(
    ...
    for symbol in result_symbols {
        ...
        if !clean_symbol.is_empty()
            && !seen.contains(&clean_symbol)
            && !is_too_similar(&clean_symbol, &query_terms)
        {
    ...
    for word in result_content_words {
        ...
        if !seen.contains(&word_lower)
            && !is_too_similar(&word_lower, &query_terms)
            && is_programming_term(&word_lower)
        {
...
pub fn extract_content_words(content: &str, limit: usize) -> Vec<String> {
    ...
    let stop_words: HashSet<&str> = [
        ...
        "this", "that", "these", "those", "it", "its", "if", "then", "else", "when", "where",
        "while", "how", "what", "which", "who", "whom", "why", "not", "no", "yes", "all", "each",
        "every", "both", "few", "more", "most", "other", "some", "such", "only", "own", "same",
        "so", "than", "too", "very", "just", "also", "now", "here", "there", "new", "old", "first",
        "last", "long", "great", "little", "own", "other", "old", "right", "big", "high", "small",
        "next", "early", "young", "important", "public", "bad", "true", "false",
    ]
    .into_iter()
    .collect();
...
mod tests {
    ...
    fn test_generate_suggestions_from_query() {
        ...
        let has_related = suggestions.iter().any(|s| {
            s == "authentication" || s == "authorization" || s == "login" || s == "session"
        });
    ...
    fn test_generate_suggestions_with_symbols() {
        ...
        assert!(suggestions.iter().any(|s| s == "auth" || s == "session" || s == "authentication"));
        // Should include cleaned symbols
        assert!(suggestions.iter().any(|s| s == "user" || s == "session" || s == "authenticate"));
    }
    ...
    fn test_extract_content_words() {
        let content = "The user authentication service handles login requests and validates credentials against the database.";
