  ...
  fn ensure_loaded(&mut self, lang: Language) {
    ...
    if let Some(grammar) = self.get_grammar(lang) {
      ...
      if parser.set_language(&grammar).is_ok() {
        ...
        self
          .queries
          .insert(lang, super::languages::load_queries(lang, &grammar));
      }
    }
  }
...
"#;
  ...
  fn test_parser_reuse_across_files() {
    ...
    let rust2 = "use tokio::fs; fn b() { helper2(); }";
    ...
    assert!(imports2.contains(&"tokio::fs".to_string()));
...
"#;
    ...
    let defs = parser.extract_definitions(modified, Language::Rust);
