...
; Methods inside impl blocks with generic type - capture just the type name
(impl_item
  type: (generic_type
    type: (type_identifier) @parent)
  body: (declaration_list
...
"#;
  ...
  #[test]
  fn test_rust_method_parent_detection() {
    let content = r#"
struct UserRepo {
    db: Database,
}
...
impl<T: Clone> GenericRepo<T> {
    fn clone_item(&self, item: &T) -> T {
        item.clone()
    }
}
"#;
    ...
    // Find the 'save' method and verify its parent is 'UserRepo'
    ...
    assert_eq!(
      ...
      Some("UserRepo"),
      "save method should have UserRepo as parent"
    );

    // Find the 'load' method and verify its parent is 'UserRepo'
    ...
    assert_eq!(
      ...
      Some("UserRepo"),
      "load method should have UserRepo as parent"
    );

    // Find the 'clone_item' method from generic impl and verify its parent
    let clone_method = defs.iter().find(|d| d.name == "clone_item");
    assert!(
      clone_method.is_some(),
      "should find clone_item method, defs: {:?}",
      defs
    );
    assert_eq!(
      clone_method.unwrap().parent.as_deref(),
      Some("GenericRepo"),
      "clone_item method should have GenericRepo as parent"
    );

    // Verify standalone struct has no parent
    let struct_def = defs.iter().find(|d| d.name == "UserRepo");
    assert!(struct_def.is_some(), "should find UserRepo struct");
    assert_eq!(struct_def.unwrap().parent, None, "struct should have no parent");
  }
}
