...
use crate::db::{
  ...
  schema::segment_accumulators_schema,
};
...
impl ProjectDb {
  /// Save or update a segment accumulator (upsert via merge_insert)
  #[tracing::instrument(level = "trace", skip(self, accumulator), fields(session_id = %accumulator.session_id))]
  pub async fn save_accumulator(&self, accumulator: &SegmentAccumulator) -> Result<()> {
    ...
    // Upsert by session_id (only one active per session)
    let mut merge_insert = table.merge_insert(&["session_id"]);
...
mod tests {
  ...
  use super::*;
  use crate::{config::Config, db::ProjectDb, domain::project::ProjectId};
