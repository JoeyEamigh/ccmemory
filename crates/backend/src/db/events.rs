...
impl ProjectDb {
  /// Log an event
  #[tracing::instrument(level = "trace", skip(self, event), fields(event_type = %event.event_type.as_str()))]
  ...
  #[tracing::instrument(level = "trace", skip(self, events), fields(count = events.len()))]
